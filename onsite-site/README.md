# onsite.uz — сайт ONSITE (антискаланты CLEAN RO)
...
python3 build.py
```
Готовый сайт появляется в папке `site/`. Её содержимое загружается в корень хостинга onsite.uz.
...
1. Контакты компании (сейчас стоят контакты с sdalimov.uz)
2. Характеристики CLEAN RO и CLEAN RO-1 из TDS: внешний вид, pH, плотность, фасовка, срок хранения, чем продукты отличаются друг от друга (ищите `TODO` в `build.py`)
3. Картинка для соцсетей 1200×630 с логотипом (сейчас стоит фото)
4. PDF с TDS и SDS, если хотите выложить их для скачивания
